...
function calculateDistance(coordinateX, coordinateY) {
  return Math.sqrt(
    Math.pow(Math.abs(coordinateX[0] - coordinateY[0]), 2) +
      Math.pow(Math.abs(coordinateX[1] - coordinateY[1]), 2)
  );
}
...
function mostPowerFullLinkStation(point) {
  ...
  var bestlinkStations = linkStations
...
points.map(mostPowerFullLinkStation).map(function (value) {
  if (!value.station) {
    ...
    console.log(
      ...
      value.point[1],
      " is ",
      value.station.coordinates[0],
      ",",
      value.station.coordinates[1],
      " with power ",
      value.station.power.toFixed(2)
    );
  }
});
